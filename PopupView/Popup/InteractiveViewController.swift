...
//  PopupView
//
//  Created by ARHAMSOFT on 17/04/2019.
...
extension InteractiveViewController: UIGestureRecognizerDelegate {
    ...
    func showInteractive() {
        ...
        UIView.animate(withDuration: 0.25, animations: {
            if self.maskType == .black {
                ...
                window.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.7)
            } else if self.maskType == .lightBlur {
                window.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.4)
            }
            self.window?.layoutIfNeeded()
        })
    }
    ...
    private func didPan(_ y: CGFloat) {
        ...
        if self.maskType == .black && window != nil{
            ...
            self.window?.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 1 - alpha)
        }
    }
